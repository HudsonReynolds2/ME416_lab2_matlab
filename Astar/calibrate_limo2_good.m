...
if best_transform > 0
    fprintf('\n✓ Auto-selected transform %d: %s\n', best_transform, transforms(best_transform).name);
    calib.transform = best_transform;
else
    fprintf('\n⚠ Could not auto-detect transform. Please select manually.\n');
    calib.transform = input('Enter transform number (1-4): ');
    
    if calib.transform < 1 || calib.transform > 4
        error('Invalid transform number!');
    end
end
